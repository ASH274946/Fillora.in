...
- **Environment Protection**: Sensitive API keys (Gemini, NVIDIA) are managed via environment variables.
- **Secure Storage**: Authentication tokens are stored using device-level encryption (Keychain/Keystore).
- **Data Privacy**: Local database content is encrypted at rest using AES-256.
- **Keystore Security**: Production keys and certificates (`*.jks`, `*.pem`) are strictly ignored.

### Environment Configuration

To run the application with full AI capabilities, you must provide your API keys during build or run:

```bash
flutter run --dart-define=GEMINI_API_KEY=your_key_here --dart-define=NVIDIA_API_KEY=your_key_here
```
...
<div align="center">
  ...
  <p>Made with ❤️ by [Ash](https://portfolio.buildwithash.dev/)</p>
</div>
