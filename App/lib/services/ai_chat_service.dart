...
class AiChatService {
  ...
  Future<String> getResponse(String userMessage, Map<String, dynamic>? context, {List<Map<String, dynamic>>? conversationHistory}) async {
    try {
      ...
      if (context != null && context['userLanguage'] != null) {
        final langCode = context['userLanguage'];
        String languageName = 'English';
        if (langCode == 'te') languageName = 'Telugu';
        if (langCode == 'hi') languageName = 'Hindi';
        if (langCode == 'ta') languageName = 'Tamil';
        
        if (langCode != 'en') {
          systemPrompt += "\n\nIMPORTANT LANGUAGE INSTRUCTION:\n"
              "The user is speaking in $languageName. "
              "Although I am sending you their message in English (translated), "
              "you should structure your response as if you are speaking to a $languageName speaker. "
              "Keep the tone appropriate for that culture. "
              "However, your actual output MUST still be in ENGLISH. "
              "My system will translate your English response back to $languageName for the user. "
              "So, just reply in clear, simple English that translates well.";
        }
      }
